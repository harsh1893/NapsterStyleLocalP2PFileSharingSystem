...
def server_talk(s):
	...
	res_list=[]
	i=0
	print("--->Entering Evaluation Testing 2<---")
	s.sendall(pickle.dumps("WHEN"))
	user_fileName="file"
	if pickle.loads(s.recv(4096))=="START":
		while i<1000:
			#get user input 
			...
			try:
				...
				raise SystemExit('Link to the server failed.')
			tot_time=time.time()-time_send
			res_list.append(tot_time)
			i+=1
		
		F = open("response_3.csv",'w') 
		for res in res_list:
			F.write((str(res)+", "))
		F.close()
		print("--->Testing 2 Done. Data saved in CSV file.<---")
